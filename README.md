<!--suppress HtmlDeprecatedAttribute HtmlDeprecatedTag, XmlDeprecatedElement -->
<div align="center"><center>

<!--suppress CheckImageSize -->
<img alt="Icon" width=100 src="./common/src/main/resources/assets/searchstats/icon.png">

## SearchStats

Adds a search bar to the statistics screen to quickly find specific entries.

[![Environment](https://img.shields.io/badge/Environment-Client-blue?logo=data:image/png;base64,)]()
[![Latest Minecraft](https://img.shields.io/modrinth/game-versions/ExI7GmJi?label=Latest%20Minecraft&color=%2300AF5C&logo=data:image/png;base64,)](https://modrinth.com/project/ExI7GmJi/versions)

[![Loader](https://img.shields.io/badge/Available%20for-Fabric-dbd0b4?logo=data:image/png;base64,)](https://fabricmc.net)
[![Loader](https://img.shields.io/badge/Available%20for-NeoForge%201.21%2B-f16436?logo=data:image/png;base64,)](https://neoforged.net)

[![Available on Modrinth](https://img.shields.io/modrinth/dt/ExI7GmJi?label=Available%20on%20Modrinth&logo=modrinth&logoColor=%2300AF5C)](https://modrinth.com/project/ExI7GmJi)
[![Available on CurseForge](https://img.shields.io/curseforge/dt/1148085?label=Available%20on%20CurseForge&logo=curseforge)](https://curseforge.com/minecraft/mc-mods/searchstats)
[![Available on GitHub](https://img.shields.io/github/downloads/TerminalMC/SearchStats/total?label=Available%20on%20GitHub&logo=github&logoColor=white)](https://github.com/TerminalMC/SearchStats)

</center></div>

### About

Simply type your query and the statistics list will be filtered for matching entries, allowing you
to easily find anything.

<table style="width:100%;">
  <tr>
    <td style="width:50%;"><img alt="In-game statistics screen 1" src="./assets/images/stats_01.png" style="width:100%;"></td>
    <td style="width:50%;"><img alt="In-game statistics screen 2" src="./assets/images/stats_02.png" style="width:100%;"></td>
  </tr>
</table>

### Related

- [ExportStats](https://modrinth.com/project/kyhaDyaG) by TerminalMC
  - Adds a button to the statistics screen to export data to a text file.
- [Better Statistics Screen](https://modrinth.com/project/n6PXGAoM) by TheCSDev
  - Completely overhauls the statistics screen and makes it more useful.

### Credits

- SearchStats is based on [StatsSearch](https://modrinth.com/project/jM41BciC) by gbl (
  discontinued),
  and includes code reused under license.

### Contact

[![Discord](https://img.shields.io/discord/1103153365216669797?logo=discord&label=Discord%20Server&color=%235865F2)](https://discord.terminalmc.dev)

[![Issues](https://img.shields.io/github/issues/TerminalMC/SearchStats?logo=github&label=GitHub%20Issues)](https://github.com/TerminalMC/SearchStats/issues)

[![License](https://img.shields.io/github/license/TerminalMC/SearchStats?label=License&logo=github&logoColor=white)](https://github.com/TerminalMC/SearchStats/blob/HEAD/LICENSE.txt)
